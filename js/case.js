...
fetch(assets/metadata/kasus/${file}.json).then(response => response.json()).then(data => {
	...
	if (!data.premium) {
		...
		banner.innerHTML = < h3 > 🔒Konten Premium < /h3> <p> Buka akses Premium untuk mempelajari: </p > < ul > < li > Patofisiologi < /li> <li>Pemeriksaan Penunjang</li > < li > Rencana Perawatan < /li> <li>Follow Up</li > < li > Key Points < /li> </ul >
			<
			button id = "premium-btn"
		class = "btn btn-primary" > Buka Premium < /button>; 
	...
	nextBtn.disabled = currentId === cases.length;
